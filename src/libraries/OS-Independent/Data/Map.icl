...
get k m = go k m
  where
    go _ Tip = Nothing
    go k (Bin _ kx x l r) = case lexOrd k kx of
      LT -> go k l
      GT -> go k r
      EQ -> Just x
...
member k m = go k m
  where
    go _ Tip = False
    go k (Bin _ kx _ l r) = case lexOrd k kx of
      LT -> go k l
      GT -> go k r
      EQ -> True
...
find k m = go k m
  where
    go _ Tip = abort "Map.!: given key is not an element in the map"
    go k (Bin _ kx x l r) = case lexOrd k kx of
      LT -> go k l
      GT -> go k r
      EQ -> x
...
findWithDefault v k m = go v k m
  where
    go def _ Tip = def
    go def k (Bin _ kx x l r) = case lexOrd k kx of
      LT -> go def k l
      GT -> go def k r
      EQ -> x
...
putWithKey f k v m = go f k v m
  where
    go :: (k a a -> a) k a (Map k a) -> Map k a | < k
...
putLookupWithKey :: (k a a -> a) k a (Map k a)
                    -> (Maybe a, Map k a) | < k
putLookupWithKey f k v m = go f k v m
  where
    go :: (k a a -> a) k a (Map k a) -> (Maybe a, Map k a) | < k
...
updateWithKey f k m = go f k m
  where
    go :: (k a -> Maybe a) k (Map k a) -> Map k a | < k
    go _ _ Tip = Tip
...
updateLookupWithKey f k m = go f k m
 where
   go :: (k a -> Maybe a) k (Map k a) -> (Maybe a,Map k a) | < k
...
alter f k m = go f k m
  where
    go :: ((Maybe a) -> Maybe a) k (Map k a) -> Map k a | < k
    go f k Tip = case f Nothing of
               Nothing -> Tip
               Just x  -> singleton k x

    go f k (Bin sx kx x l r) = case lexOrd k kx of
...
bin k x l r
  = Bin (mapSize l + mapSize r + 1) k x l r
...
valid t
  = balanced t && ordered t && validmapSize t
...
balanced t
  = case t of
      Tip            -> True
      Bin _ _ _ l r  -> (mapSize l + mapSize r <= 1 || (mapSize l <= delta*mapSize r && mapSize r <= delta*mapSize l)) &&
...
validmapSize t
  ...
  where
    realmapSize t`
      = case t` of
          Tip            -> Just 0
          Bin sz _ _ l r -> case (realmapSize l,realmapSize r) of
...
foldlStrict f x xs = go x xs
  where
    go z []     = z
    go z [x:xs] = let z` = f z x in go z` xs
...
filterWithKey :: (k a -> Bool) (Map k a) -> Map k a | Eq k & gLexOrd{|*|} k
...
fromList :: !w:[x:(!k,u:v)] -> y:(Map k u:v) | Eq k & gLexOrd{|*|} k, [x y <= u, w <= x, w <= y]
fromList []         = newMap
fromList [(k,v):xs] = put k v (fromList xs)

gFromList :: !w:[x:(!k,u:v)] -> y:(Map k u:v) | == k & gLexOrd{|*|} k, [x y <= u, w <= x, w <= y]
gFromList []         = newMap
gFromList [(k,v):xs] = gPut k v (gFromList xs)

putList :: !w:[x:(!k,u:v)] !w:(Map k u:v) -> y:(Map k u:v) | Eq k & gLexOrd{|*|} k, [x y <= u, w <= x, w <= y]
putList [] map         = map
putList [(k,v):xs] map = putList xs (put k v map)

gPutList :: !w:[x:(!k,u:v)] !w:(Map k u:v) -> y:(Map k u:v) | == k & gLexOrd{|*|} k, [x y <= u, w <= x, w <= y]
gPutList [] map         = map
gPutList [(k,v):xs] map = gPutList xs (gPut k v map)

delList :: ![k] !w:(Map k u:v) -> y:(Map k u:v) | Eq k & gLexOrd{|*|} k, [w y <= u, w <= y]
delList list map = seq [\map -> snd (delU key map) \\ key <- list] map

gDelList :: ![k] !w:(Map k u:v) -> y:(Map k u:v) | == k & gLexOrd{|*|} k, [w y <= u, w <= y]
