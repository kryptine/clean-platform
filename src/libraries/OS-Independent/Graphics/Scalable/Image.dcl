...
/** circle d = image:
		@image renders a circle of diameter (maxSpan [zero,@d]), hence the span box of @image is (maxSpan [zero,@d])
		wide and high.
*/
circle     :: !Span -> Image m
...
/** xline w = image:
		...
		This is relevant when using the Line(Start/Mid/End)Marker attributes.
*/
xline      :: !Span -> Image m
...
/** polyline []:
	...
	polyline offsets = image:
		...
		When using the Line(Start/Mid/End)Marker attributes, these points determine the direction of the lines.
*/
...
/** scale fx fy image = image`:
		@image` renders @image after scaling the width of @image with factor (maxSpan [zero,@fx]) and the
